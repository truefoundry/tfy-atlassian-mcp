...
> **🚀 TrueFoundry Fork**: This is a fork of the original [mcp-atlassian](https://github.com/sooperset/mcp-atlassian) project by TrueFoundry.
>
> **✨ New Feature Added**: Simplified OAuth authentication using HTTP Authorization Bearer tokens - you can now authenticate by simply setting the `ATLASSIAN_OAUTH_CLOUD_ID` environment variable along with `JIRA_URL`/`CONFLUENCE_URL` to enable the respective tools, without needing the full OAuth flow setup.
...
#### C. OAuth 2.0 with Bearer Token (Cloud) - **New & Simplified**

> [!TIP]
> **TrueFoundry Enhancement**: This simplified OAuth method allows you to use existing OAuth Bearer tokens without setting up the full OAuth flow.

If you already have an OAuth Bearer token from your Atlassian app or another OAuth integration:

1. Set the `ATLASSIAN_OAUTH_CLOUD_ID` environment variable with your Atlassian Cloud ID
2. Set `JIRA_URL` and/or `CONFLUENCE_URL` to enable the respective tools
3. The MCP server will use the token from the `Authorization` header passed by your client

This method is perfect for environments where you already have OAuth tokens and want to avoid the complexity of the full OAuth setup.

#### D. OAuth 2.0 Authentication (Cloud) - **Advanced**
