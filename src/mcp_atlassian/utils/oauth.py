...
class OAuthConfig:
    ...
    def is_token_expired(self) -> bool:
        ...
        """
        # If we have a cloud ID, we don't need to check for token expiration as we are using a passthrough token
        if self.cloud_id:
            return False
    ...
    def from_env(cls) -> Optional["OAuthConfig"]:
        ...
        if not all([client_id, client_secret, redirect_uri, scope]) and not cloud_id:
            return None
        if not all([client_id, client_secret, redirect_uri, scope]) and cloud_id:
            return cls(
                cloud_id=cloud_id,
            )
